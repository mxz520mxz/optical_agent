...
def design_initial_structure(
    ...
    Returns a dict with:
      - lens_config: DeepLens-compatible JSON dict
    ...
    # If lens_type given, override template selection
    override_map = {
...
def run_optimization(
    ...
    """
    Run DeepLens gradient-based optimization on the given lens config.

    Returns updated lens_config dict plus optimization log.
    """
    lrs = learning_rates or [1e-3, 1e-4, 1e-1, 1e-4]

    if not DEEPLENS_AVAILABLE:
        return _mock_optimization(lens_config, iterations)

    # Write config to temp file so GeoLens can load it
    ...
    result_dir = os.path.join(
        RESULTS_DIR, f"opt_{int(time.time())}"
    )
    os.makedirs(result_dir, exist_ok=True)

    try:
        device = "cuda" if (DEEPLENS_AVAILABLE and torch.cuda.is_available()) else "cpu"
        ...
        return {
            "lens_config": optimized_config,
            "result_dir": result_dir,
            "iterations_run": iterations,
            "status": "success",
            "summary": f"Optimization complete ({iterations} iterations). Results saved to {result_dir}",
        }
...
def evaluate_lens(lens_config: dict) -> dict:
    """
    ...
    try:
        ...
        # Run full analysis (captures printed metrics)
...
def add_lens_element(
    ...
    """
    Insert a new lens element into the lens structure.

    Args:
        lens_config: Current lens configuration
    ...
    Returns:
        Updated lens_config with new element inserted
    """
    ...
    # Choose curvatures based on element_type
    curvature_sets = {
    ...
    r_elem = foclen / (2 * fnum) * 1.1  # slightly larger than aperture
    ...
    else:  # middle: before aperture
        insert_at = aperture_idx if aperture_idx >= 0 else len(surfaces) // 2

    # Determine d position for new element
    if insert_at == 0:
        d_start = 0.0
        # Shift all existing surfaces forward
        for s in surfaces:
            s["d"] += thickness + 1.0
        # Adjust d_next of new back surface to reach the first existing surface
        ...
        last["d_next"] = 1.0  # air gap before new element
        d_next_back = 5.0 * scale  # short BFD extension
        config["d_sensor"] += thickness + d_next_back
    else:
        prev = surfaces[insert_at - 1]
        d_start = prev["d"] + prev.get("d_next", 1.0)
        gap = prev["d_next"]
        prev["d_next"] = gap / 2  # split gap
        d_next_back = gap / 2 - thickness
        if d_next_back < 0.5:
            d_next_back = 0.5
            config["d_sensor"] += thickness + 0.5
        # Shift surfaces after insertion point
        for s in surfaces[insert_at:]:
            s["d"] += thickness + (gap / 2 - gap)  # net shift
    ...
    return {
        ...
        "summary": (
            ...
            f"Lens now has {n_elements} elements, "
            f"{len(surfaces)} surfaces."
        ),
    }


def remove_lens_element(lens_config: dict, element_index: int) -> dict:
    """
    Remove a lens element (pair of surfaces) from the lens.
    ...
    Returns:
        Updated lens_config with element removed
    """
    config = copy.deepcopy(lens_config)
    surfaces = config["surfaces"]

    # Find element boundaries: element_index-th glass element
    ...
    # Merge d_next of preceding surface
    if start_surf_idx > 0:
        surfaces[start_surf_idx - 1]["d_next"] = merged_d_next

    # Remove the element surfaces and shift positions
...
def save_lens(lens_config: dict, filename: str = "final_lens", fmt: str = "json") -> dict:
    """
    Save the lens configuration to a file.

    Args:
        lens_config: Lens configuration dict
...
def generate_report(lens_config: dict, metrics: dict) -> dict:
    """
...
def _extract_metrics(lens) -> dict:
    ...
    # RMS spot size at multiple field positions
    try:
        rms_vals = {}
        for field_pct, label in [(0, "0%"), (0.7, "70%"), (1.0, "100%")]:
            ...
            rms_vals[label] = float(loss.item()) * 1000  # convert mm to µm
...
def _mock_optimization(lens_config: dict, iterations: int) -> dict:
    """Return a slightly perturbed config when DeepLens is unavailable."""
    config = copy.deepcopy(lens_config)
    # Slightly adjust curvatures to simulate optimization
    ...
    return {
        ...
        "summary": (
            f"[MOCK] Optimization simulated for {iterations} iterations. "
...
def _mock_evaluation(lens_config: dict) -> dict:
    ...
    # More surfaces → better correction (rough heuristic)
...
def _count_elements(surfaces: list) -> int:
    """Count the number of distinct glass elements (consecutive glass surfaces)."""
    count = 0
    in_glass = False
    for s in surfaces:
        mat2 = s.get("mat2", "air")
...
def _find_glass_elements(surfaces: list) -> list:
    ...
    start_idx is the first surface entering glass,
    end_idx is the last surface exiting glass.
    """
    elements = []
    start = None
    for i, s in enumerate(surfaces):
